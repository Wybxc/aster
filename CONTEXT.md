...
A **build outcome** records published pages and generated endpoints separately, together with collected warnings and total elapsed time. Internal assets and copied public files are not reported as authored routes. Build stages emit tracing spans; the CLI renders their progress and duration immediately and consistently for build, watch, and dev commands, while library callers choose whether to install a subscriber. Build modules decide whether an operation succeeds and preserve diagnostic context. Successful init and build commands return outcomes; the terminal adapter renders them and the CLI maps command results to process exit status in one place.
